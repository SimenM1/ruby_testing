...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        first_invalid_input = '!'
        second_invaild_input = '@'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(first_invalid_input, second_invaild_input, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        min = game_input.instance_variable_get(:@minimum)
        max = game_input.instance_variable_get(:@maximum)
        expect(game_input.verify_input(min, max, valid_input)).to eq(3)
      end
    end
  ...
  describe '#display_turn_order' do
    ...
    subject(:order_game) { described_class.new(1,10) }
    let(:order_search) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(order_search).to receive(:make_guess)
      allow(order_search).to receive(:update_range)
      allow(order_game).to receive(:display_guess)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { order_game.display_turn_order(order_search) }.to change { order_game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(order_search).to receive(:make_guess).once
      order_game.display_turn_order(order_search)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(order_search).to receive(:update_range).once
      order_game.display_turn_order(order_search)
    end
