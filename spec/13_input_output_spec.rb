...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    subject(:new_round) { described_class.new(5, '2')}

    context 'when user guess is not correct' do
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(new_round).not_to be_game_over
      end
    end
  end
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        user_input = '12'
        unvertified_input = input_check.verify_input(user_input)
        expect(unvertified_input).to eq(nil)
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        incorrect_value_one = '11'
        incorrect_value_two = '-5'
        valid_input = '3'
        allow(game_loop).to receive(:player_input).and_return(incorrect_value_one, incorrect_value_two, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      ...
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        else_phrase = "That was hard. It took you 6 guesses!\n"
        expect { game_three.final_message }.to output(else_phrase).to_stdout
      end
    end
  end
end
